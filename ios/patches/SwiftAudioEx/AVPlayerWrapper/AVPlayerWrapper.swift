...
class AVPlayerWrapper: AVPlayerWrapperProtocol {
    ...
    // MARK: - Gapless pre-loading properties
    ...
    func preload(url: URL, options: [String: Any]? = nil) {
        // Don't preload the same URL twice
        if let existingUrl = preloadedUrl, existingUrl == url {
            return
        }
        ...
        pendingAsset.loadValuesAsynchronously(forKeys: playableKeys) { [weak self] in
            ...
            DispatchQueue.main.async {
                // Ensure this is still the asset we want
                guard self.preloadedAsset === pendingAsset else { return }
                ...
                guard pendingAsset.isPlayable else { return }
        ...
        // Also pre-load metadata in parallel (non-blocking)
        let metadataKeys = ["commonMetadata", "availableChapterLocales", "availableMetadataFormats"]
        pendingAsset.loadValuesAsynchronously(forKeys: metadataKeys, completionHandler: {})
    }
    ...
    func load() {
        ...
        if state != .failed,
           ...
           let readyItem = preloadedItem {
            ...
            self.avPlayer.replaceCurrentItem(with: readyItem)
            self.startObservingAVPlayer(item: readyItem)
        ...
        // MARK: Normal load path (no pre-loaded item available)
        if (state == .failed) {
            recreateAVPlayer()
        } else {
            clearCurrentItem()
