...
export const fetchAllTracks = async (): Promise<Song[]> => {
	...
	// console.log(`Fetching Plex tracks from: ${url}`);

	const res = await fetch(url);
	const text = await res.text();
	console.log(`Plex Fetch Status Code: [${res.status}] from ${url}`);
