...
import { getMetadataFromUri } from '@/utils/song';
import { getMetadataFromNative } from '@/utils/nativeMeta';
      ...
      if (result.assets?.length) {
        ...
        for (const [index, file] of result.assets.entries()) {
          ...
          importedSongs.push({
            ...
            artwork:
              metadata.artwork || 'https://upload.wikimedia.org/wikipedia/commons/3/3e/Speaker_Icon.svg',
            uri: destination,
          });
        }
  ...
  const renderSongItem = ({ item }: { item: Song }) => (
    <Pressable onPress={() => playSong(item)} style={styles.songItem}>
      <View style={styles.artworkContainer}>
        ...
        {item.id === currentSong?.id && <MusicVisualizer isPlaying={playbackState.state === State.Playing} />}
      </View>
      <ThemedView
        style={[
